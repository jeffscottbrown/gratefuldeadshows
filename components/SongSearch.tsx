...
export default function SongSearch({ songs }: { songs: Song[] }) {
  const [query, setQuery] = useState('');

  const q = query.trim().toLowerCase();
  const filtered = q ? songs.filter((s) => s.title.toLowerCase().includes(q)) : null;

  // Build A-Z groups for the unfiltered view
        ...
        <div>
          ...
          {filtered.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
              {filtered.map(({ id, title, count }) => (
                <SongLink key={id} id={id} title={title} count={count} />
              ))}
            </div>
          )}
        </div>
        ...
        <>
          ...
          {letters.map((letter) => (
            <div key={letter} id={`letter-${letter}`} className="mb-8 scroll-mt-20">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
                {byLetter.get(letter)!.map(({ id, title, count }) => (
                  <SongLink key={id} id={id} title={title} count={count} />
                ))}
              </div>
            </div>
...
function SongLink({ id, title, count }: { id: number; title: string; count: number }) {
  return (
    <Link
      href={`/songs/${id}`}
    ...
    >
      ...
      <span className="text-xs text-gray-500 shrink-0 group-hover:text-gray-300">
        {count}×
      </span>
    </Link>
  );
